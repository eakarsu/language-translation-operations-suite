const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:5820';
...
const adminCookie = await login('admin@language-translation-operations.local', 'admin123');
const managerCookie = await login('manager@language-translation-operations.local', 'manager123');
const analystCookie = await login('analyst@language-translation-operations.local', 'analyst123');
